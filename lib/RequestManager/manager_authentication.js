...
var postLogin = function(callback){
    ...
    var body = {"username": config.roomManagerCredentials.username, "password": config.roomManagerCredentials.password, "authentication": "local"};
...
var postLoginIncorrect = function(type, callback){
    ...
    switch(type){
        case "incorrectPassword":
            ...
            body = {"username": config.roomManagerCredentials.username, "password": password, "authentication": "local"};
            break;
        case "incorrectAccount":
            account = generator.generateValues();
            body = {"username": account, "password": config.roomManagerCredentials.password, "authentication": "local"};
            break;
        case "incorrectSyntax":
            body = {"username": "]"+config.roomManagerCredentials.username, "password": config.roomManagerCredentials.password, "authentication": "local"};
            break;
        case "missRequeriment":
            body = {"username": config.roomManagerCredentials.username, "authentication": "local"};
            break;
    }
    var dataRequest = {"Authorization" : "", "body": body};
    request.buildRequest(method.post, endPoint, dataRequest, function(err, res){
